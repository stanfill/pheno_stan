...
dtmArray <- matrix(0,6,gid_obs)
dtmArray[1,] <- temp2011$dtm
dtmArray[2,] <- hot2011$dtm
dtmArray[3,] <- temp2012$dtm
dtmArray[4,] <- hot2012$dtm
dtmArray[5,] <- temp2013$dtm
dtmArray[6,] <- hot2013$dtm
...
check_Inits <- function(){
  list(tmin=0,topt=26,tmax=34,
       sigma_dth=runif(1,3,10),sigma_dtm=runif(1,3,10),
       tth_g=rep(850,nyears),tthm_g=rep(900,nyears),
       mu_tth=rnorm(1,1000,10),sig_tth=runif(1,1,4),
       mu_tthm=rnorm(1,1000,10),sig_tthm=runif(1,1,4), ppsen=30)
}
...
f1 <- stan(file="multitemp_pheno_wt_ppsen.stan",data=pheno_dat_gid,init=check_Inits,chains=1, iter=1)
#f2 <- stan(fit=f1,data=pheno_dat_gid,init=check_Inits,chains=1, iter=1)

p_triangular$ppsen <- 30
p_triangular$vrn$base <- 0
p_triangular$vrn$opt <- 26
p_triangular$vrn$max <- 34

pheno(weather_temperate_2011,p_triangular)
pheno(weather_heat_2011,p_triangular)

pheno(weather_temperate_2012,p_triangular)
pheno(weather_heat_2012,p_triangular)

pheno(weather_temperate_2013,p_triangular)
pheno(weather_heat_2013,p_triangular)
