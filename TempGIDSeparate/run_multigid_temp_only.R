...
pheno_dat_gid <- list(ndays=ncol(weatherMat), nobs=gid_obs,ngid=ngids,
                      ...
                      tth_g=rep(850,ngids),tthm_g=rep(900,ngids))
...
f1 <- stan(file="multigid_temp_only.stan",data=pheno_dat_gid,init=initial_multigid,chains=1, iter=1)
seed <- 12345
num_core  <-  2
...
multiGID_fit <- stan(file="multigid_temp_only.stan", data=pheno_dat_gid, algorithm="NUTS",
