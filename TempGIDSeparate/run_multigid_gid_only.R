...
initial_multigid <- function(){
  list(sigma_dth=runif(1,3,10),sigma_dtm=runif(1,3,10),
       tth_g=rnorm(nyears,1000,1),tthm_g=rnorm(nyears,1000,1), ppsen=runif(1,25,70))
}
...
f1 <- stan(file="multigid_gid_only.stan",data=pheno_dat_gid,init=initial_multigid,chains=1, iter=1)
seed <- 12345
num_core  <-  2
